<!-- 
/*Name: Nitish Mudgal
* AndrewId : nmudgal
* Date: 10 Oct 2016
* Course No : 08672
*/
 -->
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<html>
<head>
	<meta http-equiv="cache-control" content="no-cache">
	<meta http-equiv="pragma" content="no-cache">
	<title>HW4 - User and Favorite URLs</title>
	<style>
		.menu-head {font-size: 10pt; font-weight: bold; color: black; }
		.menu-item {font-size: 10pt;  color: black }
    </style>
</head>

<body>
<%@ page import="edu.cmu.cs.webapp.hw4.databean.UserBean" %>

<table cellpadding="4" cellspacing="0">
    <tr>
	    <!-- Banner row across the top -->
        <td width="130" bgcolor="#99FF99">
        <td bgcolor="#99FF99">&nbsp;  </td>
        <td width="500" bgcolor="#99FF99">
            <p align="center">
	            <c:if test="${empty title}">
	            	<font size="6" style="fontweight: bold;">HW4 - User and Favorite URLs</font>
	            </c:if>
	            <c:if test="${not empty title}">
	            	<font size="6" style="fontweight: bold;">${title}</font>
	            </c:if>
			</p>
		</td>
    </tr>
	
	<!-- Spacer row -->
	<tr>
		<td bgcolor="#99FF99" style="font-size:5px">&nbsp;</td>
		<td colspan="2" style="font-size:5px">&nbsp;</td>
	</tr>
	
	<tr>
		<td bgcolor="#99FF99" valign="top" height="500">
			<!-- Navigation bar is one table cell down the left side -->
            <p align="left">
            <c:if test="${empty user}">
            	<span class="menu-item"><a href="login.do">Login/Register</a></span><br/><br>
				<span class="menu-head">Favorites from:</span><br/>
				<c:forEach var="u" items="${userList}">
					<span class="menu-item">
						<a href="list.do?emailAddress=${u.emailAddress}">
							${u.firstName} ${u.lastName}
						</a>
					</span>
					<br/>
				</c:forEach>
			</p>
            </c:if>
            <c:if test="${not empty user}">
            	<span class="menu-head">${user.firstName} ${user.lastName}</span><br/>
				<span class="menu-head">Manage Your Account</span><br/>
					<span class="menu-item"><a href="favorite.do">View/Add Favorites</a></span><br>
					<span class="menu-item"><a href="deletefavorite.do">Delete Favorites</a></span><br>
				<span class="menu-item"><a href="change-pwd.do">Change Password</a></span><br/>
				<span class="menu-item"><a href="logout.do">Logout</a></span><br/>
				<span class="menu-item">&nbsp;</span><br/>
				<span class="menu-head">Favorites from:</span><br/>
				<c:forEach var="u" items="${userList}">
					<c:if test="${user.emailAddress != u.emailAddress}">
						<span class="menu-item">
							<a href="list.do?emailAddress=${u.emailAddress}">
								${u.firstName} ${u.lastName}
							</a>
						</span>
						<br/>
					</c:if>
					
				</c:forEach>
            </c:if>			
			</p>
		</td>
		
		<td>
			<!-- Padding (blank space) between navbar and content -->
		</td>
		<td  valign="top">
